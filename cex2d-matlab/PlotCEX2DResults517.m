...
saverawdata             = 1; % set to 1 to save csv files of the data
...
textline=fgetl(fileID);
%while isempty(strfind(textline,'Idens')) % Idens is on line 23966
...
if savefigures == 1
    ...
    saveas(iplot,pfname+'/Beamlet_and_screen_currents_vs_density.fig')
end
...
if savefigures == 1
    ...
    saveas(iplot,pfname+'/Screen_grid_transparency_vs_density.fig')
end
iplot=iplot+1;
...
if savefigures == 1
    ...
    saveas(iplot,pfname+'/Min_on-axis_potential_vs_density.fig')
end
iplot=iplot+1;
...
if savefigures == 1
    ...
    saveas(iplot,pfname+'/Beamlet_radius_at_accel_grid_vs_density.fig')
end
...
if savefigures == 1
    ...
    saveas(iplot,pfname+'/Beam_divergence_vs_density.fig')
end
iplot=iplot+1;

% Figure 6
if backstreamtoggle == 1
    fprintf('Plotting Electron Backstreaming\n')
    ...
    for j=1:nbackstreamtimes
        ...
        if plotbackstreamingcurves==1
            ...
            ann2.FontSize=14;
            if savefigures == 1
                %print([pfname+'/Backstreaming_current_vs_Vaccel_t',num2str(round(backstreamtime(j))),'khr'],'-dpng')
                %saveas(iplot,[pfname+'/Backstreaming_current_vs_Vaccel_t',num2str(round(backstreamtime(j))),'khr.fig'])
                continue
            end
            iplot=iplot+1;
        end
    ...
    if savefigures == 1
        ...
        saveas(iplot,pfname+'/Backstreaming_limit_vs_t.fig')
    end
    iplot=iplot+1;
...
if exist('griderosiondata') && sum(sum(griderosiondata)) > 0
    ...
    if savefigures == 1
        ...
        saveas(iplot,pfname+'/Screen_grid_erosion.fig')
    end
    iplot=iplot+1;
...
%         saveas(iplot,pfname+'/Accel_grid_erosion.fig')
%     end
    iplot=iplot+1;
end
...
%r               = linspace(0, accelgridholeradius, length(vr));
z_sp            = znode(ind_Vmin);
%fprintf(znode(ind_Vmin));
...
if savefigures == 1
    ...
    saveas(iplot,pfname+'/Tracer_trajectories.fig')
end
%iplot=iplot+1;
...
if savefigures == 1
    print(pfname+'/VM','-dpng')
    saveas(iplot,pfname+'/VM.fig')
    %export_fig(pfname+'/VM', -pdf )
end
...
vals = {'Variable', 'Value', 'Units';
        ...
        'Accel Voltage', Va, 'V';
        'Plume Voltage', Vc, 'V';
        'Beamlet Current Density', beamlet_Jb, 'mA';
