...
while isempty(strfind(textline,'Geometry'))
    textline=fgetl(fileID);
end
...
while isempty(strfind(textline,'Upstream Ion')) && isempty(strfind(textline,'RunTime'))
    ...
    if stoptime>0
        ...
        while isempty(strfind(textline,'End CEXIon'))
            ...
            i=i+1;
            %here
            %CEXiondata(i,:,j)=sscanf(textline,'%f');
            %textline=fgetl(fileID);
            %i=i+1;
        end
    end
    ...
    for j=1:nbackstreamtimes
        ...
        if plotbackstreamingcurves==1
            ...
            if savefigures == 1 && j == nbackstreamtimes 
                print('Plots/Backstreaming_current_vs_Vaccel_t','-dpng')
                %saveas(iplot,'Plots/Backstreaming_current_vs_Vaccel_t.fig')
                %saveas(iplot,'Plots/Beam_divergence_vs_density.fig')
            end
            subplot(1,2,1)
            legend()
...
if savefigures == 1
    print('Plots/Divergence_angle_histogram','-dpng')
    saveas(iplot,'Plots/Divergence_angle_histogram.fig')
end
iplot=iplot+1;
...
if stoptime>0
    ...
    hold on
    [C,h] = contour(axialnodes,radialnodes,potentials'-vaccel,'LineColor','k','ShowText','on');
