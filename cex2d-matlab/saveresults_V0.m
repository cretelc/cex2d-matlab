...
%filename = 'OutputDatabulk_mixi_A261.txt';
...
ebstime(1)=sscanf(textline, '%*s %*s %*s %*s %*s %f %*s');

textline   = fgetl(fileID);
ebsheaders = textline;
textline   = fgetl(fileID);
ebsheaders = strcat(ebsheaders, textline);
ebsheaders = split(ebsheaders, '  ');
ebsheaders = ebsheaders(~cellfun('isempty', ebsheaders));
ebsheaders = strtrim(ebsheaders).';

i=1;
textline = fgetl(fileID);
while ~contains(textline, ' End EBS Data')
    ebsdata(i,:) = sscanf(textline, '%f %f %f %f').';
    textline = fgetl(fileID);
    i=i+1;
end

while ~contains(textline, 'Neutral Gas Density')
    textline=fgetl(fileID);
end
...
while ~contains(textline, 'rbirth')
    textline = fgetl(fileID);
end
CEXiondata_headers = sscanf(textline, '%s %s %s %s %s %s %s %s %s %s');
CEXiondata_headers2 = split(textline, '\t');

while ~contains(textline, 'CEXIon Location, Velocity and Kinetic Energy')
    textline = fgetl(fileID);
end
textline=fgetl(fileID); % step into float lines
% Initiate CEXIon Data
i=1;
...
while ~contains(textline, 'Iteration Data')
    %t=1;
    if contains(textline, 'Node masses')
    ...
    if contains(textline, 'Node mass loss rates')
