...
j = 1;
jbackstream = 1;
while ~contains(textline,'Upstream Ion') && ~contains(textline,'RunTime')
    
    while ~contains(textline,'Electron') && ~contains(textline,'Neutral Gas')
        % skip through until you get a line containing Electron or Neutral
        % Gas
        textline=fgetl(fileID);
    end
    % read in electron backstreaming data if it exists for this time step
    if contains(textline,'Electron')
        ...
        while ~contains(textline,'End EBS')
            ...
            %writematrix(backstreamdata, strcat(results_folder,'/','backstreamdata.xls' ), 'Sheet', jbackstream, 'WriteMode', 'Append')
    ...
    while ~contains(textline,'Electron') && isempty(strfind(textline,'Neutral Gas')) && isempty(strfind(textline,'Upstream Ion')) && isempty(strfind(textline,'RunTime'))
        textline=fgetl(fileID);
    end
    j=j+1;
end
...
while ~contains(textline, 'Iteration Data')
    if contains(textline, 'Node masses at time')
